...
class File(models.Model):
      ...
      message  = models.ForeignKey(Message, related_name='Сообщение', on_delete=models.CASCADE)
...
class Group_message(models.Model):
    owner  = models.ForeignKey(Profile, on_delete=models.CASCADE, verbose_name=u"Пользователь", related_name = 'otprav', blank=True, null=True)
    users = models.ManyToManyField(Profile,  verbose_name = "Получатели почты", related_name = 'users')
    message = models.ForeignKey(Message, verbose_name='Message', related_name = 'sdsd', on_delete = models.CASCADE, blank=True, null=True )
    have_read = models.ManyToManyField(Profile, verbose_name= "Кто прочел", related_name = 'dsds', blank=True)
